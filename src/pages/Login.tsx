...
import { loginUser } from '../api/auth'; // C++ Backend'e bağlanacak API fonksiyonumuz
...
const Login: React.FC = () => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // C++ backend'e giriş isteği atılır
      const data = await loginUser(email, password);
      console.log("Giriş başarılı, Token alındı!");
      
      // Token'ı tarayıcıya kaydet
      // "Beni Hatırla" seçiliyse LocalStorage (kalıcı), değilse SessionStorage (tarayıcı kapanana kadar) kullanılabilir.
      // Şimdilik genel kullanım için LocalStorage'a kaydediyoruz:
      localStorage.setItem('jwt_token', data.token || "dummy_token");
      
      // Giriş başarılıysa Dashboard'a (Kanban/Chat alanına) geç
      navigate('/dashboard');
    } catch (err: any) {
      console.error("Giriş hatası:", err);
      // Backend'den dönen hata mesajını veya varsayılan mesajı state'e kaydet
      setError(err.response?.data?.message || "E-posta veya şifre hatalı. Lütfen tekrar deneyin.");
    } finally {
  ...
  const handleGoogleAuth = () => {
    ...
    console.log("Google ile giriş tetiklendi");
  };
  ...
  return (
    <div className="min-h-screen bg-[#0f0c29] flex items-center justify-center p-4 relative overflow-hidden font-sans">
      ...
      <div className="absolute top-[10%] right-[20%] w-[30%] h-[30%] bg-blue-600/20 blur-[120px] rounded-full pointer-events-none"></div>
      <div className="absolute bottom-[10%] left-[20%] w-[30%] h-[30%] bg-purple-600/20 blur-[120px] rounded-full pointer-events-none"></div>

      {/* Cam Efektli (Glassmorphism) Giriş Formu */}
      <div className="relative z-10 w-full max-w-md bg-white/5 backdrop-blur-xl border border-white/10 rounded-3xl p-10 shadow-2xl">
        ...
        <div className="text-center mb-8">
          ...
          <h2 className="text-3xl font-extrabold text-white mb-2">{t('login_title')}</h2>
          <p className="text-gray-400 text-sm">{t('login_desc')}</p>
        </div>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div>
            <label className="block text-gray-300 text-sm mb-2 font-medium">{t('login_email')}</label>
            <input 
              type="email" 
              value={email}
          ...
          <div>
            <label className="block text-gray-300 text-sm mb-2 font-medium">{t('login_pass')}</label>
            <input 
              type="password" 
              value={password}
          ...
          <div className="flex items-center">
            <input 
              ...
              className="w-4 h-4 bg-black/30 border-white/10 rounded text-blue-600 focus:ring-blue-500 focus:ring-offset-gray-900 cursor-pointer"
            />
            <label htmlFor="remember" className="ml-2 block text-sm text-gray-400 cursor-pointer hover:text-gray-300 transition">
              {t('remember_me')}
            </label>
          </div>
          ...
          <button 
            type="submit" 
            disabled={loading}
            className="w-full py-4 bg-blue-600 hover:bg-blue-500 disabled:bg-blue-800 disabled:cursor-not-allowed text-white rounded-xl font-bold transition shadow-lg shadow-blue-500/30 mt-6 flex justify-center items-center"
          >
            {loading ? (
        ...
        <div className="mt-8 text-center text-sm text-gray-400">
          {t('login_no_account')}{" "}
          <button onClick={() => navigate('/register')} className="text-blue-400 font-bold hover:text-blue-300 transition">
