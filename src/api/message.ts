...
export const sendMessage = async (channelId: string, content: string, attachments?: string[]) => {
  const { data } = await apiClient.post(`/channels/${channelId}/messages`, { content, attachments });
  return data;
};
...
export const createThread = async (messageId: string, name: string) => {
  const { data } = await apiClient.post(`/messages/${messageId}/threads`, { name });
  return data;
};
...
export const removeReaction = async (messageId: string, emoji: string) => {
  const { data } = await apiClient.delete(`/messages/${messageId}/reactions/${emoji}`);
  return data;
};